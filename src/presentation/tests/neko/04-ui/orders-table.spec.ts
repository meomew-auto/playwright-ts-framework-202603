...
test.describe('Orders Page - API-UI Comparison', () => {
  ...
  test('TC_11: So sánh danh sách mã đơn UI với API', async ({ ordersPage, orderApi }) => {
    ...
    const apiResponse = await orderApi.getOrdersData({ page: 1, limit: uiOrderIds.length });
  ...
  test('TC_12: So sánh tổng tiền UI với API', async ({ ordersPage, orderApi }) => {
    // 1. UI: Lấy dữ liệu từ table
    const uiData = await ordersPage.getTableData(['mã đơn', 'tổng tiền']);

    // 2. API: Lấy orders
    const apiResponse = await orderApi.getOrdersData({ page: 1, limit: uiData.length });
  ...
  test('TC_13: Filter UI cancelled và verify với API', async ({ ordersPage, orderApi }) => {
    ...
    const apiResponse = await orderApi.getOrdersData({ status: 'cancelled', limit: uiOrderIds.length });
  ...
  test('TC_14: Verify customer name UI khớp với API', async ({ ordersPage, orderApi }) => {
    // 1. API: Lấy order đầu tiên
    const apiOrders = await orderApi.getOrdersData({ limit: 1 });
