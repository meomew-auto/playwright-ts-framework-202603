 ...
 *
 * 🔗 LIÊN KẾT:
 * - Dùng: @fixtures/neko (asRole, chatPage, chatService)
 ...
 * - Multi-Role: role.fixture.ts (tạo isolated POMs + Services qua asRole)
 */
...
test.describe('Chat Realtime — Multi-role', () => {
  test('TC01. Admin và Manager chat realtime với nhau', async ({ asRole }) => {
    // ─── Setup ──────────────────────────────────────────────────────────
    // Khởi tạo 2 vai trò độc lập với Browser Context & Session riêng biệt
    ...
    // ─── Đợi cả 2 users kết nối WebSocket & xuất hiện online ─────────────
    await expect.poll(
      async () => {
        const data = await admin.chatApi.getOnlineUsers();
    ...
    await admin.chatPage.openChatWithUser(MANAGER_USER_ID);
    ...
    await admin.chatPage.sendMessage(adminMessage);
    ...
    await admin.chatPage.expectMessageReceived(managerReply);
    Logger.info('✅ Admin đã nhận được tin nhắn từ Manager');
  });

  test('TC02. Cả 2 roles online → API trả về 2 users', async ({ asRole }) => {
    // ─── Setup ──────────────────────────────────────────────────────────
    const admin = await asRole('admin');
    const manager = await asRole('manager');
    ...
    // ─── Admin check (dùng chatApi từ asRole) ───────────────────────
    const adminView = await admin.chatApi.getOnlineUsers();
    ...
    // ─── Manager check (dùng chatApi từ asRole) ─────────────────────
    const managerView = await manager.chatApi.getOnlineUsers();
