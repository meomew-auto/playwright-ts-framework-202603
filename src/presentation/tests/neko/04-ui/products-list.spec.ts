...
test.describe('Danh sách sản phẩm @read @smoke', () => {
  ...
  test('TC_06: Điều hướng về trang chủ khi click nav home', async ({ productsPage }) => {
    await productsPage.clickNavHome();
    await productsPage.expectNavigatedToHome();
  });
  ...
  test('TC_08: Điều hướng tới chi tiết sản phẩm khi click thẻ', async ({ productsPage }) => {
...
test.describe('Phân trang sản phẩm @read', () => {
  ...
  test('TC_12: Chuyển trang tiếp theo khi có phân trang', async ({ productsPage }) => {
    const hasPagination = await productsPage.hasPagination();
    ...
    // Chờ trang cập nhật danh sách sản phẩm mới qua auto-retry assertion
    await expect(async () => {
