 ...
 */
const BeanSpecificationsSchema = z.object({
...
const ProductCreateSchema = z.object({
  name: z
    ...
    .default(
      () =>
        `Test Product ${Date.now()}_${Math.floor(Math.random() * 100000)}`,
    ), // Tên mặc định có timestamp và random suffix
...
export type BeanSpecifications = z.infer<typeof BeanSpecificationsSchema>;
...
export type ProductInput = Partial<ProductCreate>;
