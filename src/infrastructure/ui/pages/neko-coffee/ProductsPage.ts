...
export class ProductsPage extends BasePage {
  ...
  private readonly pageLocators = {
    ...
    navHome: (page: Page) => page.locator('header').getByRole('link', { name: /Trang chủ|Home/i }),
    navShop: (page: Page) => page.locator('header').getByRole('link', { name: /Cửa hàng|Shop/i }),
    navAbout: (page: Page) => page.locator('header').getByRole('link', { name: /Về chúng tôi|About/i }),
    navOrderTracking: (page: Page) => page.locator('header').getByRole('link', { name: /Tra cứu đơn|Track Order/i }),
    navRegister: (page: Page) => page.locator('header').getByRole('link', { name: /Đăng ký|Register/i }),
    navLogin: (page: Page) => page.locator('header').getByRole('link', { name: /Đăng nhập|Login/i }),

    // Page Title / Heading
    pageTitle: (page: Page) => page.getByRole('heading', { name: /Cửa hàng|Shop|Store/i, level: 1 }),

    // Breadcrumb
    breadcrumbHome: (page: Page) => page.locator('a[href="/"], a[href="/en"], a[href="/vi"]').filter({ hasText: /Trang chủ|Home/i }),
  ...
  async expectOnPage(): Promise<void> {
    await expect(this.page).toHaveURL(/(?:\/vi|\/en)?\/products/, { timeout: 15000 });
    await expect(this.element('pageTitle')).toBeVisible({ timeout: 15000 });
    // Wait for at least one product card to be visible (web-first assertion)
    await expect(this.element('productCards').first()).toBeVisible({ timeout: 15000 });
  }

  /** Xác minh đã điều hướng về trang chủ */
  async expectNavigatedToHome(): Promise<void> {
    await expect(this.page).toHaveURL(/(?:\/|\/vi|\/en)$/);
  }

  /** Xác minh đã điều hướng tới trang chi tiết sản phẩm */
  async expectOnProductDetailPage(): Promise<void> {
    await expect(this.page).toHaveURL(/(?:\/vi|\/en)?\/products\/.+/);
  }
